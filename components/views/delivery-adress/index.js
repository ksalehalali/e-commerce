...
function DeliveryPageContent() {
  ...
  const {
    ...
    process.env.NEXT_PUBLIC_HOST_API + process.env.NEXT_PUBLIC_DELETE_ADRESS,
    "post",
    {},
    false
  );
  ...
  return (
    <Container>
      ...
      {!loading && data?.status === true && data?.description?.length > 0 && (
          ...
          <FlexDiv gap={20} responsive wrap="true">
            {data?.description?.map((item) => (
              <MainAdress
                key={item.id}
                data={item}
                isSelected={selected?.id === item.id ? true : false}
