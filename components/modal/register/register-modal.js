...
function RegisterModal({ visible, onClose }) {
  ...
  const fetchRegister = useCallback(async (values) => {
    ...
    try {
      const { data: res } = await axios.post(
        process.env.NEXT_PUBLIC_HOST_API + process.env.NEXT_PUBLIC_REGISTER,
        {
          ...values,
        }
      );
      ...
      if (res?.status === true) {
        ...
        setError(
          res?.description ?? "Something went wrong ! Please try again later."
        );
      }
    } catch (e) {
      setLoading(false);
  ...
  return (
    ...
    >
      ...
      >
        <FormBody>
          {error && <Alert showIcon description={error} type="error" />}
          <Form.Item
            name={"UserName"}
